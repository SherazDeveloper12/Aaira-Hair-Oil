            ...
            <AnimatePresence>
              {cartIsOpen &&
                ...
                <motion.div
                  ...
                  className='bg-white text-black  fixed right-0 top-0 h-screen w-80 lg:w-100 shadow-lg '>
                  <div className='w-full h-full flex flex-col'>
                    <div className='flex justify-between items-center p-2 lg:p-4 border-b border-primary'>
                      <h2 className='text-xl text-primary lg:text-2xl font-bold '>Your Cart</h2>
                      <X size={20} onClick={() => dispatch(toggleCart())} className='hover:text-primary cursor-pointer' />
                    </div>
                    {items.length > 0 ? <div className='flex-1 flex flex-col gap-1 lg:gap-2 px-1 lg:px-2 pt-3 lg:pt-6 pb-4 lg:pb-8 bg-white overflow-auto'>
                      {items.map((item: any) => (
                        <div key={item.product._id} className='flex justify-between items-center bg-secondary/90 rounded-xl shadow-md gap-1 lg:gap-2  border-collapse p-1 lg:p-2 hover:translate-y-1 duration-300 ease-in'>
                      ...
                      <div className='flex flex-col justify-center items-center h-full gap-1 lg:gap-2'>
                        ...
                        <button onClick={() => dispatch(toggleCart())}
                          className='cursor-pointer px-4 py-2 bg-primary text-white rounded hover:bg-primary/80'>Start Shopping</button>
                      </div>
                    }

                    <div className='p-2 lg:p-4 border-t border-primary flex flex-col gap-2 lg:gap-4'>
                      <div>
                      ...
                      <button
                        ...
                        className='w-full px-2 lg:px-4 py-1 lg:py-2 bg-primary cursor-pointer text-white rounded hover:bg-primary/80'>Proceed to Checkout</button>
                    </div>
