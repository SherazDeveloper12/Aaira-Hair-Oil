    ...
    return (
        <div>
            ...
            <label htmlFor="imageuploader"
                ...
                className={`relative text-gray-400 flex flex-col  border-dashed border-2 ${required && value === "" ? "border-red-500" : "border-gray-300"}  rounded cursor-pointer bg-primary/90 hover:border-black hover:text-black transition-all duration-300 ${size && size === 'sm' ? 'size-12' : size === 'md' ? 'size-36' : 'size-66 md:size-100'}   overflow-hidden ${ImgUrl ? 'p-0' : 'justify-center items-center'}`}>
                {ImgUrl ? (<div className='relative w-full h-full'>
                    ...
                    <div className={`${size && size === 'sm' ? 'hidden' : size === 'md' ? 'text-xs' : 'text-lg'} absolute inset-0 flex flex-col justify-center items-center text-white transition-all duration-300`}>
                    ...
                    <p className={`${size && size === 'sm' ? 'hidden' : size === 'md' ? 'text-xs' : 'text-lg'} ${imgboxhovered ? 'text-white' : 'text-white'}`}>Upload Image here</p>
                </>
                }
            </label>
